...
import { selectedVisuals, visualImageUrl, visualMetadata } from "../data/selectedVisuals";
      ...
      <ul className="selected-visuals__rail" id="selected-visuals-rail" ref={railRef}>
        {items.map((item, index) => (
          <li className="selected-visuals__item" key={item.src} style={{ "--visual-ratio": item.width / item.height } as CSSProperties}>
            <button className="selected-visuals__work" type="button" aria-haspopup="dialog" aria-label={`View ${item.title}`} onClick={event => {
              openerRef.current = event.currentTarget;
              setActiveIndex(index);
            }}>
              <img src={visualImageUrl(item.src)} alt={item.alt} width={item.width} height={item.height} loading="lazy" decoding="async" />
              <span className="selected-visuals__caption" aria-hidden="true">
                <span className="selected-visuals__title">{item.title}</span>
