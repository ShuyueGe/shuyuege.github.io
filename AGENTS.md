...
## Required reading
Read the relevant sections of `case-study-handoff/visual-design-guidelines.md` for significant visual, layout, interaction, responsive, typography, or case-study work. A precisely specified local style fix does not require rereading the full guide.
Read the corresponding final handoff when working on project facts, copy, structure, or asset selection/replacement: `case-study-handoff/bookthing-final.md` for BookThing and `case-study-handoff/patsy-final.md` for Patsy. Reuse already-read guidance unless it changed or the task needs additional context. Content Lock and factual constraints below always apply.
...
Critics evaluate visual execution only. They must not recommend copy changes; discard any such recommendation and solve the issue visually. Every future critic invocation must include both content-lock rules in the "Exact critic prompt" in `case-study-handoff/visual-design-guidelines.md`. A content restoration request does not authorize a new visual iteration, commit, or push.
...
## Visual work
Significant redesigns must use the **Independent Design Critic Workflow** in `case-study-handoff/visual-design-guidelines.md`. Determine scope by affected behavior and shared dependencies, not changed line count. Precisely specified local style fixes do not trigger independent critique or broader redesign. For substantial visual changes, use a fresh-context independent design critic when the current Codex environment supports one. Prefer a strong vision-capable OpenAI model / reviewer configuration available in the environment. The critic must receive only screenshots, reference imagery when applicable, and the fixed critic rubric. Never claim that a specific critic model was used unless the runtime actually confirms that model.

The implementer must not label same-context self-assessment as independent review. If a fresh separate critic is unavailable, perform a normal visual audit, label its score "self-review / not independent", and report that independent review remains unavailable. Self-review cannot satisfy an explicitly requested independent review or score threshold; otherwise finish the scoped implementation and relevant checks with this limitation disclosed. Do not install external critic tooling or configure paid external API access.

## Implementation
Inspect `git status` and the relevant source or documents before editing. Inspect routing when navigation/routes are affected, and `package.json` when build commands or dependencies matter. Preserve unrelated and pre-existing user work. Preserve working hash routes, accessibility, semantic HTML, responsive behavior, reasonable performance, and factual accuracy. Do not silently change navigation labels, the portfolio wordmark, or route slugs.

Run `npm run build` after application code, styles, or build configuration changes. It includes `tsc -b && vite build`, so do not repeat the same type check; there is no lint script. For documentation/skill-only changes, validate the edited files without building the site. After checks pass, repeat or broaden them only for new changes, failures, or unresolved concerns. Never manually edit generated `dist/` output or modify `node_modules/`. Rebuild from source.

For local visual fixes, inspect the affected area on desktop (1440 px) and mobile (390 px), including alignment, wrapping, overflow, and content preservation. Include 820 px when a breakpoint or intermediate layout may be affected. Shared styles/components, typography systems, breakpoint changes, and structural redesigns require all three widths across affected pages. Test image enlargement, focus, reduced motion, navigation, and image loading when those behaviors or their dependencies are affected. Use `scripts/design-review.mjs` for repeatable screenshot evidence, setting `REVIEW_PAGES` to affected page names (`home`, `bookthing`, `patsy`) unless the change is site-wide; output is ignored in `.design-review.local/`.

Complete the requested behavior and relevant checks before delivering. Fix build failures, overflow, lost content, and accessibility/functionality regressions caused by the change; report unrelated pre-existing issues without expanding scope. Content Lock, factual accuracy, and preservation of existing user work cannot be marked not applicable to avoid a difficult check. Do not restart historical design iterations merely because an earlier score was below its target.
