...
Use the verification scope in `AGENTS.md`: local fixes need the affected area at desktop 1440 px and mobile 390 px, with 820 px when intermediate layouts may be affected; shared or structural changes need all three widths across affected pages. Additionally check 320 px when minimum-width overflow is a concern. Design explicit column collapse, not proportional desktop shrinkage. Check affected text measure, type scale, image crops, gallery width, spacing, sticky/fixed elements, navigation, 44 px interaction targets, and first-screen hierarchy. Mobile screens stack at readable widths; desktop evidence gets focused detail crops and enlargement. Do not hide all project previews behind hover on touch devices.
...
Apply this workflow to significant redesigns or an explicitly requested independent review. Precisely specified local fixes use the scoped verification in `AGENTS.md`.

1. Inspect git status and the source, handoff sections, and package scripts needed for the task; run the current site and capture a baseline before visual edits.
2. Capture fresh current screenshots of affected pages at desktop/mobile, plus full-page overviews and readable close-ups of affected sections. Include tablet in implementation QA for significant redesigns.
...
6. Implement, build, render, inspect, and request a new fresh critique. Inspect convergence after the first two iterations. Unless the user specifies another budget, allow at most five major iterations per affected page for the task. Stop earlier when two consecutive iterations do not materially improve, feedback becomes contradictory, or missing truthful content/assets block progress. Report unresolved issues and next actions honestly; do not automatically restart the loop in a later task.
7. **Implementer-only quality target, never send to the critic:** aim for 9.0/10. Unless the user explicitly requires a score threshold, completion depends on the requested changes and relevant verification, with actual scores and remaining limitations reported honestly. An explicit score threshold remains binding; if convergence or the iteration budget stops work below it, report the target as unmet rather than claiming completion or silently continuing. Fix build failures, overflow, content loss, and accessibility/functionality regressions introduced by the change before delivery. A score cannot waive Content Lock or authorize changes outside the task.

If the current environment cannot spawn a separate fresh context, report that limitation, continue normal visual inspection, and label any score **self-review / not independent**. Such a score cannot satisfy an explicitly requested independent review or score threshold. Otherwise finish the scoped implementation and relevant checks with the review limitation disclosed. Same-context self-critique is never an independent review.
...
Reuse a suitable running local server, or run `npm run dev -- --host 127.0.0.1`, then `node scripts/design-review.mjs baseline` (or a new stage name). Set `REVIEW_PAGES` to a comma-separated list of affected pages (`home`, `bookthing`, `patsy`); omit it only for site-wide checks. The script captures all three widths for the selected pages. For a local fix needing fewer widths, targeted browser inspection is sufficient. The script uses Playwright and Sharp from a tool runtime; set `PLAYWRIGHT_MODULE` and `SHARP_MODULE` to its module paths when they are not installed locally. It defaults to an installed Edge browser; override `REVIEW_BROWSER` if needed. No project dependency installation is required when the runtime provides them.
...
Use an already-available built-in image generation tool directly. Check CLI installation/version or authentication only if the chosen supported generation method actually depends on it. Do not install or reinstall a CLI merely as a prerequisite for an available built-in tool. Use diagnostics only when needed. Where sign-in is required, use **Sign in with ChatGPT**, never an API key; pause only for necessary interactive sign-in and provide precise instructions.
